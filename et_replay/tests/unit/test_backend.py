...
from et_replay.comm.backend import PyTorchDistBackend
from et_replay.comm.backend.base_backend import BackendBootstrapInfo
...
def backend(request):
    print("init backend")
    if request.param == "gloo":
        backend = PyTorchDistBackend()
    elif request.param == "mock":
        backend = MockBackend()
    else:
        raise ValueError('unexpected backend value')
    backend_bootstrap_info = BackendBootstrapInfo()
    backend_bootstrap_info.master_ip = "localhost"
    backend_bootstrap_info.master_port = 12345
    backend_bootstrap_info.world_rank = 0
    backend_bootstrap_info.world_size = 1
    backend_bootstrap_info.local_size = 1
    backend_bootstrap_info.local_rank = 0

    comm_params = {"use_ext_dist": "1",
                    "backend": "gloo"}
    backend.initialize_backend(backend_bootstrap_info, comm_params)
    return backend
